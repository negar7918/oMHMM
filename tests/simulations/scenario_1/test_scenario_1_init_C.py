...
from oMHMM.tests.simulations.scenario_1 import spamhmm_orthogonal_with_hyperparam_init_C, spamhmm_init_C
...
mhmm = spamhmm_init_C.SpaMHMM(n_nodes=1,
                              mix_dim=M,
...
mhmm = spamhmm_orthogonal_with_hyperparam_init_C.SpaMHMM(hyperparam=1, epsilon=0, n_nodes=1,
                                                         mix_dim=M,
                                                         n_components=num_of_states,
                                                         n_features=seq_len,
                                                         graph=None,
...
print(transitions.reshape(M, num_of_states, num_of_states))
print(np.exp(pi_nk2))
o_acc = accuracy_score(label2, predicted_cluster2)

print('oMHMM v_measure: {}'.format(np.mean(v_measure_oMHMM)))
print('MHMM v_measure: {}'.format(np.mean(abs(v_measure))))
