...
#from global_utils import relu, drelu, relu_normalization, reg_graph
import pickle
import time
import warnings
import math
from MHMM.tests import rand_initialization
...
class SpaMHMM(BaseEstimator):
    ...
    def init_params(self, X):
        ...
        self.mixCoef = np.reshape(np.array([.5, .5]), (1, 2))#relu_normalization(self.mixCoefUnNorm, axis=1) #  #
        ...
        if self.emission == 'gaussian':
            ...
            for m in range(self.mix_dim):
                ...
                seed = 1
                trans = rand_initialization.generate_transitions_random(seed, 2, self.n_components, .5)
