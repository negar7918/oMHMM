...
M = 2
S = 4 # 2: 2 MHMM, 2 oMHMM; 4: 12 MHMM, 12 oMHMM
...
for n in range(num_of_cells):
    ...
    predicted_cluster = np.append(predicted_cluster, np.where(cell == np.max(cell))[0][0])
...
label_1 = [1 for i in range(len(data2[:, 0]))]
for n in range(num_of_cells):
    cell = np.float64(np.exp(pi_nk[n]))
    if round(cell[0], 2) == round(cell[1], 2):
        predicted_cluster = np.append(predicted_cluster, 0)
    else:
        predicted_cluster = np.append(predicted_cluster, np.where(cell == np.max(cell))[0][0])
